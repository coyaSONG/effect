...
describe("Tool representation v2", () => {
  it("projects the encoded side before JSON Schema generation", () => {
    assert.strictEqual(Tool.getJsonSchemaFromSchema2(Schema.FiniteFromString).type, "string")
  })
  ...
  it("uses custom JSON Schema compiler annotations for schema parameters", () => {
    ...
    assert.deepStrictEqual(Tool.getJsonSchema2(tool), Tool.getJsonSchemaFromSchema2(schema))
    assert.deepStrictEqual(Tool.getJsonSchema2(tool), {
      type: "object",
      properties: {
        value: {
          type: "string",
