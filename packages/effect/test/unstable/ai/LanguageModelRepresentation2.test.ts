...
describe("LanguageModel representation v2", () => {
  it("projects the encoded side before JSON Schema generation", () => {
    assert.strictEqual(LanguageModel.defaultCodecTransformer2(Schema.FiniteFromString).jsonSchema.type, "string")
  })
  ...
  it("uses custom JSON Schema compiler annotations in the default transformer", () => {
    ...
    assert.deepStrictEqual(LanguageModel.defaultCodecTransformer2(schema).jsonSchema, {
      type: "string",
      allOf: [{ minLength: 2 }]
    })
  })
