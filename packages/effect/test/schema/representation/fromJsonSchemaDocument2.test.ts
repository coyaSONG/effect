import { JsonSchema, Schema, SchemaRepresentation2 as SchemaRepresentation } from "effect"
...
describe("fromJsonSchemaDocument2 parity", () => {
  function assertFromJsonSchema(
    ...
    deepStrictEqual(canonicalize(document), canonicalize(expectedDocument))
    const schema = SchemaRepresentation.toSchemaFromJsonSchemaDocument(jsonDocument, input.options)
    if (runtime !== undefined) {
      const live = SchemaRepresentation.fromAST(schema.ast)
      const code = SchemaRepresentation.toCodeDocument({
        representations: [live.representation],
  ...
  describe("allOf", () => {
    ...
    function assertLiteralRefinement(
      ...
      for (const literal of [valid, invalid]) {
        for (const allOf of [[refinement, { const: literal }], [{ const: literal }, refinement]]) {
          const schema = SchemaRepresentation.toSchemaFromJsonSchemaDocument(
    ...
    it("combines references and annotated stable wrappers on either side", () => {
      ...
      for (
        ...
        const document = SchemaRepresentation.fromJsonSchemaDocument(JsonSchema.fromSchemaDraft2020_12(schema))
      ...
      const aliases = SchemaRepresentation.fromJsonSchemaDocument(
      ...
      strictEqual(aliases.references.A._tag, "Suspend")
      strictEqual(aliases.references.B._tag, "String")
    })

    it("merges string content, overlapping properties and index signatures", () => {
      const string = SchemaRepresentation.fromJsonSchemaDocument(
        JsonSchema.fromSchemaDraft2020_12({
          allOf: [
      ...
      const leftContentSchema = SchemaRepresentation.fromJsonSchemaDocument(
        JsonSchema.fromSchemaDraft2020_12({
          type: "string",
      ...
      const object = SchemaRepresentation.toSchemaFromJsonSchemaDocument(
        JsonSchema.fromSchemaDraft2020_12({
          type: "object",
          additionalProperties: false,
          properties: { a: { type: "string" } },
      ...
      const optionalObject = SchemaRepresentation.fromJsonSchemaDocument(
        JsonSchema.fromSchemaDraft2020_12({
          type: "object",
          additionalProperties: false,
          properties: { a: { type: "string" } },
      ...
      const indexes = SchemaRepresentation.fromJsonSchemaDocument(
        JsonSchema.fromSchemaDraft2020_12({
          type: "object",
          additionalProperties: false,
