...
describe("SchemaRepresentation2 built-in atomic declarations", () => {
  it("emits the open declaration protocol and preserves legacy generation", () => {
    for (const entry of cases) {
      ...
      if (legacy.representation._tag === "Declaration") {
        assert.deepStrictEqual(legacy.representation.annotations?.typeConstructor, { _tag: entry.name })
  ...
  it("revives every declaration without a global registry", () => {
    ...
    const json = SchemaRepresentation2.toJsonMultiDocument(document)
  ...
  it("composes the Date declaration with a revived built-in check", () => {
    ...
    const json = SchemaRepresentation2.toJson(SchemaRepresentation2.fromAST(original.ast))
    const revived = noServices(SchemaRepresentation2.fromJson(json, {
    ...
    const document = SchemaRepresentation2.fromASTs([revived.ast])
    assert.deepStrictEqual(SchemaRepresentation2.toJsonSchemaMultiDocument(document).schemas[0], {
      type: "string",
  ...
  it("rejects a non-null declaration payload", () => {
    const json = SchemaRepresentation2.toJson(SchemaRepresentation2.fromAST(Schema.Date.ast)) as any
