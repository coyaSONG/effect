...
describe("SchemaRepresentation2 built-in Date checks", () => {
  it("emits canonical ISO payloads and target callbacks", () => {
    for (const entry of cases) {
      ...
      const legacy = SchemaRepresentation.fromAST(Schema.Date.check(check).ast)
  ...
  it("rejects valid but non-canonical ISO strings", () => {
    const original = SchemaRepresentation2.toJson(
      SchemaRepresentation2.fromAST(Schema.Any.check(Schema.isGreaterThanDate(date(0))).ast)
    )
    for (
      const value of [
