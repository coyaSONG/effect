...
function patternJsonSchema(check: SchemaAST.Filter<any>): object {
  const meta = check.annotations?.meta as { readonly regExp: RegExp }
...
describe("SchemaRepresentation2 built-in string checks", () => {
  it("emits the dual protocol from the same normalized constructor arguments", () => {
  ...
  it("rejects non-canonical parameter payloads", () => {
    ...
    for (const entry of invalid) {
      const json = SchemaRepresentation2.toJson(
        SchemaRepresentation2.fromAST(Schema.String.check(entry.check).ast)
      ) as any
