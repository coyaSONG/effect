import {
  ...
  Schema,
  SchemaGetter,
  type SchemaRepresentation2 as SchemaRepresentation
} from "effect"
import * as InternalRepresentation2 from "effect/internal/schema/representation2"
import { describe, it } from "vitest"
import { deepStrictEqual } from "../../utils/assert.ts"
import { canonicalize } from "./v2Parity.ts"

describe("fromASTs2 parity", () => {
  function assertFromASTs(schemas: readonly [Schema.Constraint, ...Array<Schema.Constraint>], expected: {
    ...
    const document = InternalRepresentation2.fromEncodedASTs(Arr.map(schemas, (s) => s.ast))
    deepStrictEqual(
