...
describe("SchemaRepresentation2 built-in object checks", () => {
  it("emits the dual protocol for property-count checks", () => {
    for (const entry of cases) {
      ...
      const legacy = SchemaRepresentation.fromAST(
        Schema.Record(Schema.String, Schema.Number).check(check).ast
      )
      assert.strictEqual(legacy.representation._tag, "Objects")
      if (legacy.representation._tag === "Objects") {
  ...
  it("emits the encoded key schema as an isPropertyNames dependency", () => {
    ...
    assert.strictEqual(check.annotations?.meta?._tag, "isPropertyNames")
    assert.deepStrictEqual(check.annotations?.representation, {
    ...
    assert.deepStrictEqual(
      check.annotations?.generation?.({ schemas: [{ runtime: "Schema.String", Type: "string" }] }),
    ...
    if (legacy.representation._tag === "Objects") {
      ...
      assert.strictEqual(legacyCheck._tag, "Filter")
      if (legacyCheck._tag === "Filter") {
        assert.strictEqual(legacyCheck.meta._tag, "isPropertyNames")
      }
    }
  })
  ...
  it("rejects non-canonical count payloads and missing propertyNames dependencies", () => {
    const countJson = SchemaRepresentation2.toJson(
      SchemaRepresentation2.fromAST(Schema.Any.check(Schema.isMinProperties(1)).ast)
    ) as any
    ...
    const propertyNamesJson = SchemaRepresentation2.toJson(
      SchemaRepresentation2.fromAST(Schema.Any.check(propertyNamesCheck()).ast)
    ) as any
