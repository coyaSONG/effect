...
describe("SchemaRepresentation2 forward IR", () => {
  it("uses the type side unless the caller projects the encoded side", () => {
    const typeDocument = SchemaRepresentation2.fromAST(Schema.NumberFromString.ast)
    const encodedDocument = SchemaRepresentation2.fromAST(SchemaAST.toEncoded(Schema.NumberFromString.ast))
  ...
  it("preserves live declaration annotations without encodedSchema", () => {
    const generation: SchemaRepresentation2.Generation.Declaration = () => ({
      runtime: "Custom",
      Type: "string"
    })
    const toJsonSchema: SchemaRepresentation2.ToJsonSchema.Declaration = () => ({ type: "string" })
    ...
    const representation = SchemaRepresentation2.fromAST(Custom.ast).representation
    ...
    assert.strictEqual(representation.annotations?.generation, generation)
  ...
  it("keeps custom checks on literals, their callbacks, dependencies and aborted state", () => {
    const toJsonSchema: SchemaRepresentation2.ToJsonSchema.Check = () => ({})
    const generation: SchemaRepresentation2.Generation.Check = () => ({
    ...
    const representation = SchemaRepresentation2.fromAST(Schema.Literal("a").check(check).ast).representation
    ...
    assert.strictEqual(persistedCheck.annotations?.generation, generation)
  ...
  it("promotes string content annotations to structural fields", () => {
    ...
    const representation = SchemaRepresentation2.fromAST(encoded).representation
  ...
  it("shares named references across roots", () => {
    ...
    const document = SchemaRepresentation2.fromASTs([shared.ast, shared.ast])
  ...
  it("extracts anonymous recursion into a reference", () => {
    ...
    const document = SchemaRepresentation2.fromAST(recursive.ast)
